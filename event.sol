...
contract MyContract{
    ...
    event Purchase(    //event are the way for external consumers to kind of thing listen that happern on smart contract
        address indexed _buyer,    //indexed is allow you to filter events that are only for certain buyers
        uint _amount
        );
    ...
    function buyToken() public payable {       //payablre is a modifier which is used to send the ether without it is not possible
        ...
        emit Purchase(msg.sender, 1);   //msg.sender -> _buyer , 1 -> _amount
    }
}
